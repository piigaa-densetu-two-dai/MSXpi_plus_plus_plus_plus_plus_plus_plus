Nextor対応！Sunrise IDE + 4096kメモリーマッパー + 漢字ロム・漢字ドライバーエミュレーターです。
...
2. sunrise_4m_kanji.binファイルの後ろに1でダウンロードしたNextorカーネル(128k)と漢字ベーシック(漢字ドライバー)ROM(32k)と漢字ROM(256k)を連結します。

unixのcatコマンドを使用する例
$ cat sunrise_4m_kanji.bin Nextor-2.1.2.SunriseIDE.MasterOnly.ROM kanjibasic.rom kanjifont.rom > firmware.bin

windowsのcopyコマンドを使用する例
>copy /b sunrise_4m_kanji.bin + Nextor-2.1.2.SunriseIDE.MasterOnly.ROM + kanjibasic.rom + kanjifont.rom firmware.bin
