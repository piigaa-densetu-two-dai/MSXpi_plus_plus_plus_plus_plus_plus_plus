Nextor対応！Sunrise IDE + 8192kメモリーマッパーエミュレーターです。(ターボRモード対応版)

USBメモリをSunrise IDEのマスター側に接続されたハードディスクとして使用する事ができます。

ご注意
ターボRモード対応ですがNextorカーネル・メモリーアクセス時にウェイトが入ります。
...
2. sunrise_8mTR.binファイルの後ろに1でダウンロードしたNextorカーネルを連結します。

unixのcatコマンドを使用する例
$ cat sunrise_8mTR.bin Nextor-2.1.2.SunriseIDE.MasterOnly.ROM > firmware.bin

windowsのcopyコマンドを使用する例
>copy /b sunrise_8mTR.bin + Nextor-2.1.2.SunriseIDE.MasterOnly.ROM firmware.bin
